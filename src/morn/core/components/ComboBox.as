...
package morn.core.components {
	...
	public class ComboBox extends Component {
		...
		protected function changeList():void {
			...
			_list.itemRender = new XML("<Box><Label mouseEnabled='true' name='label' htmlText='true' width='" + labelWidth + "' size='" + _itemSize + "' height='" + _itemHeight + "' color='" + labelColor + "' x='1' /></Box>");
...
//			}
		...
		private var _foceSelectChange:Boolean = false;
		public function set selectedIndex(value:int):void {
			if (_selectedIndex != value || _foceSelectChange) 
			{
				_foceSelectChange = false;
		...
		public function set selectedIndexFoce(value:int):void
		{
			_foceSelectChange = true;
			selectedIndex = value;
		}
		...
		private var _id2Index:Dictionary;
		private var _index2Id:Dictionary ;
		private var _dataExt:Array;
		public function set dataSouceExt(value:Object):void
		{
			var datasource:Array = [];
			_dataExt = [];
			_id2Index = new Dictionary();
			_index2Id = new Dictionary();
			if(value)
			{
				var i:int=0;
				for(var prop:String in value)
				{
					var id:String = value[prop].id;	
					datasource.push(id+"-"+value[prop].name);
					_dataExt.push(value[prop]);
					_id2Index[id] = i;
					_index2Id[i]= id;
					i++;
				}
		...
		public function getIndexById(id:*, defaultIndex:int=-1):int
		{
			if(_id2Index.hasOwnProperty(id))
				return _id2Index[id];
			return defaultIndex;
		}
		public function getIdByIndex(index:int):*
		{
			if(_index2Id.hasOwnProperty(index))
				return _index2Id[index];
			return null;
		}
		public function getDataByIndex(index:int):*
		{
			if(index<0 || _dataExt==null || _dataExt.length<=index)return;
			return _dataExt[index];
		}
